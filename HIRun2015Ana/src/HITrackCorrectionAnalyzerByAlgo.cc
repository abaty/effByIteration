...
class HITrackCorrectionAnalyzerByAlgo : public edm::EDAnalyzer {
   public:
      explicit HITrackCorrectionAnalyzerByAlgo(const edm::ParameterSet&);
      ~HITrackCorrectionAnalyzerByAlgo();
...
HITrackCorrectionAnalyzerByAlgo::HITrackCorrectionAnalyzerByAlgo(const edm::ParameterSet& iConfig):
treeHelper_(),
...
doAlgoSeparation_(iConfig.getParameter<bool>("doAlgoSeparation")),
...
HITrackCorrectionAnalyzerByAlgo::~HITrackCorrectionAnalyzerByAlgo()
{
   delete vtxWeightFunc_;
}

void
HITrackCorrectionAnalyzerByAlgo::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
  ...
  */
   ...
   std::sort( vsorted.begin(), vsorted.end(), HITrackCorrectionAnalyzerByAlgo::vtxSort );
   ...
   for(edm::View<reco::Track>::size_type i=0; i<tcol->size(); ++i){ 
     ...
     if(recSimColl.find(track) != recSimColl.end())
     {
       ...
       if( mtp->status() < 0 ) 
       {
         ...
         if(doAlgoSeparation_){
           int count=0;
           int nmuonSteps = 9;
           for(unsigned i = 0; i < algoParameters_.size()-(nmuonSteps-1); i++){
             if(algoParameters_[i]==37 && tr->originalAlgo()>=37) count++;//group all muon regional iterations together once you hit the muon regional steps
             else if( tr->originalAlgo() == algoParameters_[i] )  count++;
             if(count>0) trkCorr2D_[Form("hsec%d",i)]->Fill(tr->eta(),tr->pt(), w);
           }
         }
     ...
     else
     {
       ...
       if(doAlgoSeparation_){
         ...
         for(unsigned i = 0; i < algoParameters_.size()-(nmuonSteps-1); i++){
           ...
           if(count>0) trkCorr2D_[Form("hfak%d",i)]->Fill(tr->eta(),tr->pt(), w);
         }
       }
   ...
   { 
     ...
     if(nrec>0){
       trkCorr2D_["heff"]->Fill(tp->eta(),tp->pt(), w);
       if(doAlgoSeparation_){
         int count=0;
         int nmuonSteps = 9;
         for(unsigned i = 0; i < algoParameters_.size()-(nmuonSteps-1); i++){
           if(algoParameters_[i]==37 && algo>=37) count++;//group all muon regional iterations together once you hit the muon regional steps
           else if( algo == algoParameters_[i] )  count++;
           if(count>0) trkCorr2D_[Form("heff%d",i)]->Fill(tp->eta(),tp->pt(), w);
         }
       }
     }
...
bool
HITrackCorrectionAnalyzerByAlgo::multCuts(const reco::Track & track, const reco::Vertex & vertex)
{
...
bool
HITrackCorrectionAnalyzerByAlgo::passesTrackCuts(const reco::Track & track, const reco::Vertex & vertex)
{
...
bool 
HITrackCorrectionAnalyzerByAlgo::caloMatched( const reco::Track & track, const edm::Event& iEvent, unsigned it )
{
...
void
HITrackCorrectionAnalyzerByAlgo::initHistos(const edm::Service<TFileService> & fs)
{


  std::vector<std::string> hNames2D = { "hsim", "hrec", "hmul", "hfak", "heff", "hsec", "heff0" ,"hfak0", "hsec0", "heff1" ,"hfak1", "hsec1", "heff2" ,"hfak2", "hsec2", "heff3" ,"hfak3", "hsec3", "heff4" ,"hfak4", "hsec4", "heff5" ,"hfak5", "hsec5","heff6" ,"hfak6", "hsec6","heff7" ,"hfak7", "hsec7","heff8" ,"hfak8", "hsec8","heff9" ,"hfak9", "hsec9","heff10" ,"hfak10", "hsec10" };
...
bool
HITrackCorrectionAnalyzerByAlgo::vtxSort( const reco::Vertex &  a, const reco::Vertex & b )
{
...
void
HITrackCorrectionAnalyzerByAlgo::beginJob()
{
}

void
HITrackCorrectionAnalyzerByAlgo::endJob()
{
}

DEFINE_FWK_MODULE(HITrackCorrectionAnalyzerByAlgo);
